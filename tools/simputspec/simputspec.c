  ...
  do { // Beginning of ERROR HANDLING Loop.
    ...
    if ((strlen(par.ISISFile)>0) || (use_components>0)) {
      ...
      write_isisSpec_fits_file(par.Simput, par.ISISFile, par.ISISPrep,"",
			       par.Elow, par.Eup, par.Estep, par.plPhoIndex, par.bbkT,
			       par.flSigma, par.rflSpin, par.NH, &status);
    ...
    if (strlen(par.XSPECFile)>0) {
      ...
      write_xspecSpec_file(par.Simput, par.XSPECFile, par.Elow, par.Eup,
			   par.Estep, &status);
    ...
    if ((strlen(par.ISISFile)>0) || (use_components>0)) {
    	...
    	read_isisSpec_fits_file(par.Simput, simputspec, par.ISISFile, par.Emin, par.Emax,
    			par.plFlux, par.bbFlux, par.flFlux, par.rflFlux,&status);
    	CHECK_STATUS_BREAK(status);
